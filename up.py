...
bot_token = '7595588536:AAFAEtz2b_dd_64KkCehbwVZwH0fkPM5DNk' # Replace with your bot token
admin_id = '6769245930'  # Replace with your actual admin user ID (as a string)
GROUP_ID = '-1002487517182'  # Allowed group ID where commands work AND group cooldown applies (as a string)
...
def approve_in_private(message):
    ...
    approval_message = (
        f"🎉𝘾𝙤𝙣𝙜𝙧𝙖𝙩𝙪𝙡𝙖𝙩𝙞𝙤𝙣𝙨🎉 {target_first_name}!\n\n"
...
def handle_bgmi(message):
    ...
    if not is_admin and sender_user_id in users_needing_feedback:
        ...
        feedback_msg = (
            ...
            f"{safe_first_name} (`{sender_user_id}`),\n\n"
            f"Before starting a new attack, please send a **unique screenshot** as feedback for your previous attack."
        )
    ...
    if len(command_parts) != 4:
        ...
        usage_msg = (
            ...
            f"𝐋𝐚𝐬𝐭 𝐃𝐮𝐫𝐚𝐭𝐢𝐨𝐧 --> {MIN_ATTACK_DURATION} - {MAX_ATTACK_DURATION} seconds\n\n"
            f"Ｔｅａｍ－Ｓ４ ｏffiｃｉａｌ"
        )
    ...
    try:
        ...
        user_notification_lines = [
            ...
            f" ➤ 𝐓𝐚𝐫𝐠𝐞𝐭 `{target}",
            f" ➤ 𝐏𝐨𝐫𝐭 --> `{port}`",
            f" ➤ 𝐓𝐢𝐦𝐞 --> `{attack_time}` seconds",
        ]
        if not is_admin:
            user_notification_lines.append(f" ➤ **Attacks Remaining:** {attacks_left_after}")
            if is_in_target_group:
                 user_notification_lines.append(f" ➤ **Group Cooldown:** Now active ({COOLDOWN_TIME}s)")
        else:
...
def monitor_attack(sender_user_id):
    ...
    try:
        ...
        completion_lines = [
            ...
            f"┣• 💠𝐓𝐚𝐫𝐠𝐞𝐭 - `{target_str}`",
            f"┣• 💠𝐓𝐢𝐦𝐞 -  `{expected_duration}`s / {elapsed_time:.1f}s (Actual)",
...
def show_help(message):
    ...
    help_lines.append(f"• `/bgmi <target> <port> <time>` - Start an attack.")
    help_lines.append(f"   *(Time: {MIN_ATTACK_DURATION}-{MAX_ATTACK_DURATION}s | Port: 1-65535)*")
    help_lines.append(f"• `/check` - Show your status (attacks left, cooldowns, feedback).")
    help_lines.append(f"• `/id` - Show your User ID and current Chat ID.")
    help_lines.append(f"• `/start` - (Send in PM) Get started or request private access.")
    help_lines.append(f"• `/help` - Show this message.")

    help_lines.append("\n**📸 Feedback System:**")
    help_lines.append(f"• After an attack, you **must** send a **unique screenshot** (image) as feedback before starting the next `/bgmi`.")
    help_lines.append(f"• Send the image in any chat where the bot operates for you (Group: `{GROUP_ID}` or PM if approved).")

    if REQUIRED_CHANNELS:
        channel_links = "\n".join([f"  🔗 [{ch.replace('@', '')}](https://t.me/{ch.replace('@', '')})" for ch in REQUIRED_CHANNELS])
        help_lines.append(f"\n**📢 Channel Membership (Required for `/bgmi`):**")
        help_lines.append(f"• You must be a member of:")
        help_lines.append(channel_links)

    if is_admin:
        help_lines.append("\n**👑 Admin Commands:**")
        help_lines.append(f"• `/approve_in_private <user_id>` - Allow user private chat access.")
        help_lines.append(f"• `/remove_in_private_chat <user_id>` - Revoke private access.")
        help_lines.append(f"• `/reset <user_id>` - Reset user's limits, cooldown, feedback.")
        help_lines.append(f"• `/listapproved` - List users with private access.")
        help_lines.append(f"• *(Admin bypasses limits, cooldowns, feedback, channels)*")
        help_lines.append(f"• *(Admin gets 'Stop Attack' buttons on notifications)*")

    help_lines.append(f"\n---\n*Bot Info:*")
    help_lines.append(f"🕒 Personal Cooldown: {COOLDOWN_TIME}s")
    help_lines.append(f"⏳ Group Cooldown (in `{GROUP_ID}`): {COOLDOWN_TIME}s")
    help_lines.append(f"📊 Daily Attacks: {max_daily_attacks} per user (UTC reset)")
    # FIX: Wrap the @ mention in backticks
    help_lines.append(f"❤️ Credit: `@S4_LUCHI`")
